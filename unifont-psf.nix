...
EOF
        ...
        # Create equivalence file for bdf2psf
        cat > minimal.equiv <<'EOF'
        U+0020 U+00A0 # SPACE = NO-BREAK SPACE
        U+002D U+2010 U+2011 # HYPHEN-MINUS = HYPHEN
EOF

        # Convert to PSF1 format (256 glyphs)
        bdf2psf --psf1 --length=256 unifont.bdf minimal.equiv minimal.symb unifont.psf || true

        # Verify the conversion worked
        if [ ! -f unifont.psf ]; then
            # Try alternative conversion if first attempt failed
            bdf2psf unifont.bdf minimal.equiv minimal.symb unifont.psf
        fi
